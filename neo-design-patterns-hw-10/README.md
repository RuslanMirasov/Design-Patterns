...
Кожна дія (додати, видалити, оновити, завершити) — це окремий клас-команда, який реалізує Command через AbstractCommand. Команда сама зберігає, що треба, щоб потім відкотити дію: RemoveTaskCommand запам'ятовує видалену задачу, UpdateTaskCommand — стару копію задачі (oldTask), CompleteTaskCommand — попередній статус completed.

TaskManager створює потрібну команду і передає її в CommandHistory, а той вже виконує, скасовує і повторює команди по черзі.
...
CommandHistory тримає масив виконаних команд і індекс поточної позиції. undo() відкочує команду під поточним індексом і зсуває індекс назад, redo() зсуває вперед і виконує команду знову. Якщо після undo зробити нову дію, весь "хвіст" зі старими командами обрізається.
