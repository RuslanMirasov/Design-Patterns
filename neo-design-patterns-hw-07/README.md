...
```bash
npm start -- markdown output.md
```

створює файл `output.md`:
...
```bash
npm start -- html output.html
```

створює файл `output.html` з тією ж структурою документа, відформатованою як HTML (`<h1>`/`<h2>`, `<p>`, `<ul><li>`), обгорнутою у повну HTML-сторінку зі стилями.
